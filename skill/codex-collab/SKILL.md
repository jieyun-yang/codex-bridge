...
4. The response is free text containing findings, strengths, pressure tests, and confidence notes. Parse and render it (see "Findings format" below).
...
Codex returns free text. Claude parses it and renders into this canonical markdown:
...
Severity: `must_fix → MUST-FIX`, `should_fix → SHOULD-FIX`, `consider → CONSIDER`. Confidence prints alongside severity. For code-review, look for findings, then pressure tests. For challenge, look for issues.

When Codex lists strengths, surface them under a `## What's working` heading BEFORE the issues/findings. Render each as a bullet, no severity tags. If no strengths mentioned, omit the heading.
